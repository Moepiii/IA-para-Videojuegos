...
# ----------------------------------------------------------------------
# PROPIEDADES DE PURSUE Y AVOIDANCE 
# ----------------------------------------------------------------------
@export_group("Comportamiento")
@export var max_prediction: float = 1.0       # Max tiempo para predecir (Pursue)
...
# ----------------------------------------------------------------------
# PROPIEDADES DE LOOK WHERE YOU'RE GOING (Angular)
# ----------------------------------------------------------------------
...
# VARIABLES INTERNAS
# ----------------------------------------------------------------------
var target_evader: CharacterBody2D = null # El objetivo a perseguir
...
func _ready():
	var evaders = get_tree().get_nodes_in_group("evader")
	if evaders.size() > 0:
		target_evader = evaders[0]
		print("✅ Perseguidor listo. Objetivo: Evader.")
	else:
		print("❌ ERROR: El objetivo Evader no tiene la etiqueta 'evader'.")
		
	raycast = get_node_or_null(raycast_node_path)
	if not raycast:
		print("❌ ERROR: RayCast2D no encontrado. ¡Asigna el Path correctamente!")
	
	print("Perseguidor: Esperando tecla R para activar/desactivar la persecución.")

func _input(event):
	if event is InputEventKey and event.keycode == KEY_R and event.is_pressed():
		is_active = !is_active
		if is_active:
			print("Perseguidor activado (Persiguiendo a Evader).")
		else:
			print("Perseguidor desactivado.")
...
func get_pursue_steering(target: CharacterBody2D) -> Vector2:
	...
	return get_seek_steering(target_position)
...
func _physics_process(delta: float):
	if not is_active or not is_instance_valid(target_evader) or not raycast: 
	...
	# --- PRIORIDAD 1: OBSTACLE AVOIDANCE (Ejecutado primero)
	linear_acceleration = get_avoidance_steering()
	
	if linear_acceleration == Vector2.ZERO:
		# --- PRIORIDAD 2: PURSUE (OBJETIVO) --- (Solo se ejecuta si no hay obstáculo)
		linear_acceleration = get_pursue_steering(target_evader)
