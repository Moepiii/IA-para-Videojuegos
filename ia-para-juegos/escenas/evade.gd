...
@export var max_prediction: float = 3.0     

# Propiedad: Distancia de seguridad máxima para dejar de huir.
...
func _ready():
	...
	if players.size() > 0:
		jugador = players[0]
	print("Evasor: STOPPED. Presiona W para alternar el estado.")
...
func _input(event):
	...
	if event is InputEventKey and event.keycode == KEY_W and event.is_pressed():
		if current_state == State.STOPPED:
			...
			print("Evasor activado: EVADE")
		else:
			current_state = State.STOPPED
			print("Evasor desactivado: STOPPED")
...
func get_flee_steering(target_position: Vector2) -> Vector2:
	...
	return acceleration.normalized() * max_acceleration * -1
...
func _physics_process(delta: float):
	...
	if current_state == State.EVADE:
		# 1. COMPROBACIÓN DE DISTANCIA MÁXIMA
		var distance_to_jugador = global_position.distance_to(jugador.global_position)
		
		if distance_to_jugador < safe_distance:
			# EVADE ACTIVO: Huir si está demasiado cerca
			var predicted_target_position = get_predicted_target_position()
			linear_acceleration = get_flee_steering(predicted_target_position)
		else:
			# EVADE INACTIVO TEMPORAL: Frenar al alcanzar la distancia segura
